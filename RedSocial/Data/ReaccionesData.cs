...
namespace RedSocial.Data
{
    public class ReaccionesData
    {
        ...
        public async Task<bool> CrearReaccion(int idUser, int idPost,int idReaccion)
        {
            using var cnn = new SqlConnection(connectionstring);
            var crear = await cnn.ExecuteAsync("Insert INTO Reacciones Values (@idPost,@idUser,@idReaccion)", new {idPost,idUser,idReaccion});
        ...
        public async Task<bool> EditarReaccion(int idUser, int idPost, int idReaccion)
        {
            using var cnn = new SqlConnection(connectionstring);
            var edit = await cnn.ExecuteAsync("UPDATE Reacciones SET IdTipoReaccion= @idReaccion WHERE IdUsuario= @idUser AND IdPost= @idPosr)", new {idReaccion,idUser,idPost});
